...
final class StreamsRepository: ObservableObject {
    ...
    private func runDiscovery(requestKey: String, type: String, videoId: String) async {
        ...
        guard !enabledURLs.isEmpty else {
            // A NAS-only user with no add-ons configured still has their local
            // file — that must not read as "no add-ons configured".
            state = StreamsDiscoveryState(
                requestKey: requestKey,
                revision: state.revision &+ 1,
                groups: localGroup.map { [$0] } ?? [],
                isAnyLoading: false,
                emptyStateReason: localGroup == nil ? .noAddonsConfigured : nil,
                hasResolvedTargets: true
            )
            print("[StreamsRepo] no enabled add-ons, local=\(localGroup?.streams.count ?? 0)")
            return
        }
        ...
        // The local group (if any) is already resolved — no network round trip —
        // so it leads the list and never carries a loading state.
        let initialGroups = (localGroup.map { [$0] } ?? []) + targets.map {
            AddonStreamGroup(
                addonId: $0.addonId,
