...
public final class AetherEngine: ObservableObject {
    ...
    func setNativeScrubSeek(inFlight: Bool, target: Double?) {
        ...
        scrubLandingWatchdog = Task { @MainActor [weak self] in
            ...
            let vmStr = Self.vmBreakdownMB().map { " (RAM footprint=\($0.physFootprintMB)MB, heap=\($0.internalMB)MB)" } ?? ""
            EngineLog.emit(
                "[AetherEngine] scrub landing watch expired after \(String(format: "%.0f", budget))s"
                + vmStr + "; reporting stalled",
                category: .engine
            )
    ...
    public var displayDebugLines: [String] {
        ...
        if let softwarePerformanceLine = softwareHost?.softwarePerformanceLine {
            lines.append(softwarePerformanceLine)
        }
        return lines
    }
    ...
    private func runStartupReadinessGate(
        ...
        // Attempt 1 plays the item the load path already created; later attempts replay it fresh after swapItem.
        host.play()
        while true {
    ...
    public func load(
        ...
        // VideoToolbox capability queries can synchronously enter the system
        // decoder service. Avoid initializing the AV1 probe for other codecs,
        // and run it off @MainActor when AV1 routing actually needs it.
        let av1Available: Bool
        if detectedCodecID == AV_CODEC_ID_AV1 {
            av1Available = await Task.detached(priority: .userInitiated) {
                VTCapabilityProbe.av1Available
            }.value
        } else {
            av1Available = true
        }
    ...
    private func handleRefusedAirPlayMaster(baseline: Double, position: Double) {
        ...
        let vmStr = Self.vmBreakdownMB().map { " (RAM footprint=\($0.physFootprintMB)MB)" } ?? ""
        EngineLog.emit(
            "[AirPlay] receiver did not start in "
            + "\(String(format: "%.0f", Self.airPlayProgressWatchdogSeconds))s (clock advanced "
            + "\(String(format: "%.2f", advanced))s, no segment fetched)"
            + vmStr + "; the playlist it was handed is "
